...
public class MaxMin {
     ...
     * 解题思路：二分查找 + 从右往左模拟 要求操作后所有元素均不超过 limit
     *
     * @param nums
     * @return
     */
    ...
    public boolean checkMin(int[] nums, int limit) {
        ...
        for (int i = nums.length - 1; i > 0; i--) {
            extra = Math.max(0, nums[i] - limit + extra);
        }
    ...
    public void testMinimizeArrayValue() {
        Assert.assertEquals(5, minimizeArrayValue(new int[]{3, 7, 1, 6}));
        Assert.assertEquals(10, minimizeArrayValue(new int[]{10, 1}));
    }
}
